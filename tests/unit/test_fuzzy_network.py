...
from keras.src.models import Sequential, Functional, Model
...
class FuzzyLayerTest(testing.TestCase):
    ...
    def test_basic_flow_with_input(self):
        ...
        model.add(layers.Input(shape=(2,), batch_size=3))
        ...
        model = Sequential(
            layers=[
                layers.Input(shape=(2,), batch_size=3),
                layers.Dense(4),
                layers.Dense(5),
            ]
        )
    ...
    def test_basic_flow_with_sequential_model_as_first_layer(self):
        ...
        sequential_model = Sequential(
            [layers.Input((16, 16, 3)), layers.Conv2D(4, 3, padding="same")]
        )
